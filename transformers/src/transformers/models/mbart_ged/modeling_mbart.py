...
class MBartDecoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        self.ged_attn = MBartAttention(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True,
        )
        self.fc_attn = nn.Linear(2 * self.embed_dim, self.embed_dim) # to combine both x-attens
    ...
    def forward(
        ...
        encoder_ged_hidden_states: Optional[torch.Tensor] = None,
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states_tmp, cross_attn_weights, cross_attn_present_key_value = self.ged_attn(
                hidden_states=hidden_states_tmp,
                key_value_states=encoder_ged_hidden_states,
                attention_mask=encoder_attention_mask,
                layer_head_mask=cross_attn_layer_head_mask,
                past_key_value=None,
                output_attentions=output_attentions,
            )
            hidden_states = torch.cat((hidden_states, hidden_states_tmp), dim=-1)
            hidden_states = self.activation_fn(self.fc_attn(hidden_states))
...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # if we have ged tags, create an embedding layer to learn
        # their representations
        if hasattr(config, 'ged_num_labels'):
            self.embed_ged_tags = nn.Embedding(config.ged_num_labels, embed_dim, 0)
            # self.embed_ged_tags = nn.Embedding(config.ged_num_labels, 128, 0)
            # self.projection = nn.Linear(128 + embed_dim, embed_dim)
            # self.project_ged_tags = nn.Linear(config.ged_num_labels, embed_dim, bias=False)
    ...
    def forward(
        ...
        ged_tags: torch.LongTensor=None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if ged_tags != None:
            ged_tags_embed = self.embed_ged_tags(ged_tags)
            # ged_tags_embed = nn.functional.dropout(ged_tags_embed, p=0.1, training=self.training)
            # ged_tags_embed = self.project_ged_tags(ged_tags)

            hidden_states = inputs_embeds + embed_pos + ged_tags_embed
            # hidden_states = inputs_embeds + embed_pos
            # concat ged tags
            # hidden_states = torch.cat((hidden_states, ged_tags_embed), dim=-1)
        ...
        # )
        return BaseModelOutputGED(
            last_hidden_state=hidden_states, hidden_states=encoder_states, attentions=all_attentions,
            ged_hidden_state=ged_tags_embed
        )
...
class MBartDecoder(MBartPreTrainedModel):
    ...
    def forward(
        ...
        encoder_ged_hidden_states: Optional[torch.FloatTensor] = None,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    encoder_ged_hidden_states=encoder_ged_hidden_states,
...
class MBartModelGED(MBartPreTrainedModel):
    def __init__(self, config: MBartConfig):
        super().__init__(config)
    ...
    def get_encoder(self):
        return self.encoder
        # return self.encoder, self.ged_encoder
    ...
    def forward(
        ...
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput) and not isinstance(encoder_outputs, BaseModelOutputGED):
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_ged_hidden_states=encoder_outputs.ged_hidden_state,
...
class MBartForConditionalGenerationGED(MBartPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: MBartConfig):
        super().__init__(config)
        self.model = MBartModelGED(config)
