...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        areta_tags = [feature["areta_tags"] for feature in features] if "areta_tags" in features[0].keys() else None

        # We have to pad the areta_tags before calling `tokenizer.pad` as this method won't pad them and needs them of the
        # same length to return tensors.
        if areta_tags is not None:
            max_tags_length = max(len(l) for l in areta_tags)
            if self.pad_to_multiple_of is not None:
                max_tags_length = (
                    (max_tags_length + self.pad_to_multiple_of - 1)
                    // self.pad_to_multiple_of
                    * self.pad_to_multiple_of
                )
            ...
            for feature in features:
                ...
                remainder = [self.areta_tag_pad_token_id] * (max_tags_length - len(feature["areta_tags"]))
                # remainder = [[0] * 30 for _ in range(max_tags_length - len(feature["areta_tags"]))]

                if isinstance(feature["areta_tags"], list):
                    feature["areta_tags"] = (
                        feature["areta_tags"] + remainder if padding_side == "right" else remainder + feature["areta_tags"]
                    )
                elif padding_side == "right":
                    feature["areta_tags"] = np.concatenate([feature["areta_tags"], remainder]).astype(np.int64)
                else:
                    feature["areta_tags"] = np.concatenate([remainder, feature["areta_tags"]]).astype(np.int64)

        # import pdb; pdb.set_trace()
        # padded_areta_tags = torch.FloatTensor([feature["areta_tags"] for feature in features])

        # for feature in features:
        #     del feature["areta_tags"]
        ...
        # features["areta_tags"] = padded_areta_tags
...
@dataclass
class DataCollatorForSeq2SeqGEC:
    """
    Data collator that will dynamically pad the inputs received, as well as the labels.
    ...
    def __call__(self, features, return_tensors=None):
        ...
        ged_tags = [feature["ged_tags"] for feature in features] if "ged_tags" in features[0].keys() else None

        # We have to pad the ged_tags before calling `tokenizer.pad` as this method won't pad them and needs them of the
        # same length to return tensors.
        if ged_tags is not None:
            max_tags_length = max(len(l) for l in ged_tags)
            if self.pad_to_multiple_of is not None:
                max_tags_length = (
                    (max_tags_length + self.pad_to_multiple_of - 1)
                    // self.pad_to_multiple_of
            ...
            for feature in features:
                ...
                remainder = [self.ged_tag_pad_token_id] * (max_tags_length - len(feature["ged_tags"]))

                if isinstance(feature["ged_tags"], list):
                    feature["ged_tags"] = (
                        feature["ged_tags"] + remainder if padding_side == "right" else remainder + feature["ged_tags"]
                    )
                elif padding_side == "right":
                    feature["ged_tags"] = np.concatenate([feature["ged_tags"], remainder]).astype(np.int64)
                else:
                    feature["ged_tags"] = np.concatenate([remainder, feature["ged_tags"]]).astype(np.int64)


        padded_ged_tags = None
        if "ged_tags" in features[0].keys():
            padded_ged_tags = torch.LongTensor([feature["ged_tags"] for feature in features])

        if "ged_tags" in features[0].keys():
            for feature in features:
                del feature["ged_tags"]
        ...
        features["ged_tags"] = padded_ged_tags
