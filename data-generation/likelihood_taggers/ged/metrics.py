...
def compute_metrics(gold_list, preds_list, labels):
    ...
    #     avg_f1 += f1_score(_gold, _preds, zero_division=0)
    #     avg_precision += precision_score(_gold, _preds, zero_division=0)
    #     avg_recall += recall_score(_gold, _preds, zero_division=0)
    ...
    return {'f1': report['samples avg']['f1-score'],
...
if __name__ == '__main__':
    ...
    if args.mode == 'verbose':
        ...
        results = eval(gold_tags, pred_tags, labels)
        with open(args.output, "w") as f:
            for metric in ['f1', 'precision', 'recall']:
