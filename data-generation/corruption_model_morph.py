...
class CorruptModel:
    ...
    def build(cls, data):
        ...
        for example in data.examples:
            ...
            morph_feats = example.morph_feats

            assert len(src_tokens) == len(tgt_tokens) == len(areta_tags) == len(morph_feats)

            for i in range(len(src_tokens)):
                tgt_t, src_t, areta_tag, morph = tgt_tokens[i], src_tokens[i], areta_tags[i], morph_feats[i]

                if areta_tag != 'UC' and areta_tag != 'UNK':
                    rule = Rule.create(tgt_t, src_t, areta_tag)

                    if len(tgt_t.split()) > 1: # we have to deal with this issue in the alignment

                        assert len(morph) > 1
                        if areta_tag == 'SPLIT' and len(tgt_t.split()) == 2:
                            morph_str = json.dumps([morph[0]], ensure_ascii=False)
                            model[(areta_tag, morph_str)][rule.to_str()] += 1
                            examples[(areta_tag, morph_str)][rule.to_str()].append({'correct': tgt_t, 'corrupted': src_t})
                            counts[(areta_tag, morph_str)] = 1 + counts.get((areta_tag, morph_str), 0)
                    else:
                        morph_str = json.dumps(morph, ensure_ascii=False)
                        model[(areta_tag, morph_str)][rule.to_str()] += 1
                        examples[(areta_tag, morph_str)][rule.to_str()].append({'correct': tgt_t, 'corrupted': src_t})
                        counts[(areta_tag, morph_str)] = 1 + counts.get((areta_tag, morph_str), 0)
        ...
        #     if tag in pruned_model:
        #         pruned_counts[tag] = sum(pruned_model[tag].values())

        # return cls(pruned_model, examples, pruned_counts)
        return cls(model, examples, counts)
...
class Rule:
    ...
    def create(cls, src, tgt, areta_tag):
        ...
        if areta_tag == 'SPLIT':
            ...
            num_merges = areta_tag.split('+').count('MERGE')
            ...
            for tag, i1, i2, j1, j2 in sm.get_opcodes():
                ...
                if tag != 'equal':
                    ...
                    if tag == 'replace':
                        ...
                        elif 'MERGE' in areta_tag and num_merges > 0:
        ...
        for example in data.examples:
            ...
            for i in range(len(src_tokens)):
                ...
                if areta_tag != 'UC' and areta_tag != 'UNK':
                    ...
                    if len(tgt_t.split()) > 1: # we have to deal with this issue in the alignment
                        assert len(morph) > 1
                        # we have to deal with splits such that two words get to be mapped to one
                        if areta_tag == 'SPLIT'  and len(tgt_t.split()) == 2:
                            morph_str = json.dumps([morph[0]], ensure_ascii=False)
                            model[(areta_tag, morph_str, tgt_t)][src_t] += 1
                            counts[(areta_tag, morph_str, tgt_t)] = 1 + counts.get((areta_tag, morph_str, tgt_t), 0)
                    else:
                        morph_str = json.dumps(morph, ensure_ascii=False)
                        model[(areta_tag, morph_str, tgt_t)][src_t] += 1
                        counts[(areta_tag, morph_str, tgt_t)] = 1 + counts.get((areta_tag, morph_str, tgt_t), 0)
...
#     data = Dataset(raw_data_path='/scratch/ba63/gec/data/alignment/modeling_areta_tags_check/qalb14/corruption_data/qalb14_train.areta.txt',
#                    morph_feats_path='/scratch/ba63/gec/data/alignment/modeling_areta_tags_check/qalb14/corruption_data/train_morph.json')
#     model = CorruptModel.build(data)
#     import pdb; pdb.set_trace()