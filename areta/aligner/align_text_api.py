...
from aligner.alignment import align_words
...
def write_distances_only(distances, raw_sent, correct_sent, file_stream):
    ...
    for distance in distances:
        if distance[0] is None:
            ...
            line = line + ('\t<\t')
            line = line + "<"
            line = line + correct_sent[distance[1] - 1]
            line = line + (f'{distance}')
        elif distance[1] is None:
            file_stream.write(raw_sent[distance[0] - 1])
            file_stream.write('\t>\t')
            file_stream.write(f'\t{distance}\n')

            line = line + raw_sent[distance[0] - 1]
            line = line + '\t>\t'
            line = line + (f'\t{distance}\n')
        else:
            file_stream.write(raw_sent[distance[0] - 1])
            ...
            line = line + raw_sent[distance[0] - 1]
            line = line + '\t'
            line = line + ('=' if distance[2] == 'n' else '|')
            line = line + '\t'
            line = line + (correct_sent[distance[1] - 1])
            line = line + (f'\t{distance}\n')
...
def align_api(raw_sentences, correct_sentences):
    # if __name__ == "__main__":

    # arguments = docopt(__doc__)
    # raw_sentences = ["ف قد كان"]
    # correct_sentences = ["ف كان"]
    mode = 'basic'

    if mode == 'align':
        raw_output = open("fout" + '.align', 'w')
        correct_output = open("fout" + '.align', 'w')
        col_align_out = open("fout" + '.coAlign', 'w')
        col_align_out.write(f'RAW\CORRECT\n')
    elif mode == 'basic':
        output = open("fout2" + '.basic', 'w')
    ...
    for raw, correct in zip(raw_sentences, correct_sentences):
        ...
        if mode == 'align':
            ...
            elts = write_distances_only(alignments, raw_sent, correct_sent, output)
            all_elements.append(elts)

    if mode == 'align':
        print(f'RAW alignments are saved to align')
        raw_output.close()
        print(f'CORRECT alignments are saved to align')
        correct_output.close()
        print(f'Side by side alignments are saved to coAlign')
        col_align_out.close()
    elif mode == 'basic':
        # print(f'Basic alignments are saved to basic')
        output.close()
        lines= []
        with codecs.open("fout2.basic", "r", "utf8") as f:
            for l in f:
                lines.append(l)
        #f_n = codecs.open("fout2.basic", "r", "utf8")
        #f_text = f_n.readlines()
        #f_n.close()
    #return f_text
    return lines
