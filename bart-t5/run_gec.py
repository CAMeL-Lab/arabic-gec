...
class DataTrainingArguments:
    ...
    areta_tags: str = field(default=None, metadata={"help": "List of areta error tags labels."})
...
def main():
    ...
    # preparing areta tags if the are provided
    if data_args.areta_tags:
        areta_labels = ['<pad>'] + get_labels(data_args.areta_tags)

        areta_label_map : Dict[int, str] = {i: label for i, label in
                                            enumerate(areta_labels)}
        areta_id2label_map : Dict[str, int] = {label: i for i, label in
                                            enumerate(areta_labels)}
        num_labels = len(areta_labels)
    ...
    # Adding ARETA tags to the config if they are provided
    if data_args.areta_tags:
        config.update({'areta_id2label': areta_label_map,
                       'areta_label2id': areta_id2label_map,
                       'areta_num_labels': num_labels
                       })
    ...
    def preprocess_function(examples):
        ...
        # Converting the ARETA labels to ids if tags were provided
        if areta_tags:
            ...
            model_inputs["areta_tags"] = [features[i]['areta_labels_ids']
...
def convert_labels_to_ids(tokenizer, inputs, labels, label_map,
    ...
    the same number of subwords and same number of areta labels)
    """
    ...
    for i, (seq, seq_labels) in enumerate(zip(inputs, labels)):
        ...
        for word, label in zip(seq.split(), seq_labels.split()):
            ...
            if len(word_tokens) > 0:
                ...
                # Adding the same label to all the tokens of the word
                if not t5 or (prefix and word not in prefix.split()): # skip prefix tokens
                    if 'UNK' in label_map: # handeling unkown labels during inference
                        label_ids.extend([label_map.get(label, label_map['UNK'])] * len(word_tokens))
